...
if use_nodename_as_hostname
  node_hostname = node[:cyclecloud][:node][:name]
  node_prefix = node[:pbspro][:node_prefix]
  if node_prefix
    node_hostname = "#{node_prefix}#{node_hostname}"
  end
  ...
  execute "hostnamectl set-hostname #{node_hostname}" do
    not_if { shell_out!("hostnamectl status", { :returns => [0, 1] }).stdout =~ /Static hostname:\s+#{node_hostname}/ }
  end
end
